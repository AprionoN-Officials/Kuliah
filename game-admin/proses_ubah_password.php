...
if (isset($_POST['submit_password'])) {
    ...
    if ($baru !== $konf) {
        echo "<script>alert('Konfirmasi password baru tidak cocok!'); window.location='akun.php';</script>";
        exit;
    }
    ...
    if (password_verify($lama, $data['password'])) {
        ...
        if ($update) {
            echo "<script>alert('Berhasil! Password telah diganti.'); window.location='akun.php';</script>";
        } else {
            echo "<script>alert('Gagal menyimpan ke database.'); window.location='akun.php';</script>";
        }
